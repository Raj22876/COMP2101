...
if [ -z "$allinterfaces" ]; then 
	allinterfaces="$(ip r| grep -v default | awk '{print $3}' | sort | uniq)"
fi 
for interface in $allinterfaces; do
	...
	# To ensure your network numbers have names, add them to your /etc/networks file, one network to a line, as networkname 
